...
class Main:
    ...
    def find_path_bfs(self, edge_mat):
        ...
        for edge in edge_path:
            ...
            time_slot, score = edge.find_time_slot_fast(start_time, offset, self.cycle, self.length, self.deadline - cur_time)
            # print("time_slot", time_slot, start_time, offset)
            if time_slot < 0:
                self.path = []
                break
...
def main():
    ...
    for n in range(3, 8):
        ...
        for i in range(0, 100):
            datasets.append(f"../jhy/Ladder_NetWork/n{node_num}/{i}/")
    cur_time = time.time()
    # 创建结果文件
    write_result_init()
