...
class ActorAgent(Agent):
    ...
    def invoke_model(self, manual):
        ...
        time_slot, _ = edge.find_time_slot(self.env.tt_flow_time_record[-1], cycle, flow_length, max_delay)
...
def main():
    ...
    # actor_agent.saver_policy.restore(actor_agent.sess, "../models/typ_T02/policy/")
    ...
    while True:
        ...
        cur_time = time.time()
        if done == -1 or actor_agent.env.tt_query_id + 1 == len(actor_agent.env.tt_queries):
            ...
            # print("TT_flow", flow_number, "cycle", cycle, "per_flow_cnt_total", per_flow_cnt_total,
            #       "usage", usage,  "use time", cur_time - start_time, "delay", delay, "reward", reward)
            time_record[flow_number] = [flow_number, cycle, per_flow_cnt_total, per_flow_cnt_valid, cur_time - start_time, delay]
            '''while actor_agent.env.edge_usage() < 0.7:
                temp = list(actor_agent.env.tt_flow_to_edge.keys())
                actor_agent.env.delete_tt_flow(random.choice(temp), False)'''
    ...
    print(usage_record, len(time_record))

    # 验证调度结果
    # actor_agent.env.schedule.show()
    verify = Verify(actor_agent.env.schedule.sche)
    print(verify.judge_conflict(), (time.time() - total_time) / len(time_record))
